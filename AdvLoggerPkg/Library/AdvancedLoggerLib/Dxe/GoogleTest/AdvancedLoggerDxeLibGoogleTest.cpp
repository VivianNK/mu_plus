...
protected:
  ...
  UINTN DebugLevel;  // Error level of items top be printed
  CHAR8 *Buffer;
  UINTN NumberOfBytes;
    ...
    )
  {
    ...
    gLoggerProtocol->Signature = ADVANCED_LOGGER_PROTOCOL_SIGNATURE;
    gLoggerProtocol->Version   = ADVANCED_LOGGER_PROTOCOL_VERSION;
    */
  }
};
...
TEST_F (AdvancedLoggerWriteTest, AdvLoggerWriteSuccess) {
  EXPECT_CALL (
    ...
    .WillOnce (
       DoAll (
         SetArgPointee<2> (ByRef (gLoggerProtocol)),
         Return (EFI_SUCCESS)
         )
       );
...
TEST_F (AdvancedLoggerWriteTest, AdvLoggerWriteFailMismatchedSignature) {
  gLoggerProtocol->Signature = SIGNATURE_32('T','E','S','T');
