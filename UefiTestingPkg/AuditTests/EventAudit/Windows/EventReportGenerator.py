...
def main():
    ...
    parser.add_argument('-i', '--InputCsvFile',
                        help='Path to the input Csv file with raw data', default=None)
    parser.add_argument('-r', '--OutputHtml', dest='output_html_file',
                        help='Name of the output HTML file which will contain the boot timing info', default=None)
    ...
    with open(options.InputCsvFile) as csvfile:
        ...
        for row in reader:
            ...
            start_time = float(row[3]) / 1000000
            ...
            if function_name is None:
                logging.error(
                    'Error! address %s not found in map file' % function_addr)
            ...
            eventList.append((idCounter, event_group_guid, "TBD",
                             image_path_short, function_name, start_time, duration, tpl))
    ...
    if (html_report is not None):
        ...
        for line in template.readlines():
            ...
            if "%TO_BE_FILLED_IN_BY_PYTHON_SCRIPT%" in line:
                ...
                html_report.write("\"Model\": \"%s\"," %
                                  "TODO - get model of system?")
                html_report.write("\"UefiVersion\": \"%s\"," %
                                  "TODO - get Uefi version?")
                html_report.write("\"DateCollected\": \"%s\"," % datetime.datetime.strftime(
                    datetime.datetime.now(), "%A, %B %d, %Y %I:%M%p"))
                html_report.write(
                    "\"ReportGenVersion\": \"%s\"," % reportGenVersion)
                ...
                for event in eventList:
                    ...
                    html_report.write("{\"ID\": \"%s\",\"EventGroupGuid\": \"%s\",\"GuidValue\": \"%s\",\"ImagePath\": \"%s\",\"FunctionName\": \"%s\",\"StartTime\": \"%.6f\",\"Duration\": \"%.6f\",\"Tpl\": \"%.6f\"}" % (
