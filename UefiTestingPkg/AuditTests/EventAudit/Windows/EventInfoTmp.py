...
def parse_event_debug(lines_iter, events_called):
    ...
    try:
        for line in lines_iter:
            ...
            event.function_name = get_func_name(
                map_path, event.function_addr[2:])

            if event.function_name is None:
                print("DEBUG INFO: error - address " +
                      event.function_addr[2:] + " not found in map file")
            else:
                out_str = event.function_name + "\t" + \
                    event.time_ns + event.tpl + event.image_path + "\n"
